...
function SnailChasingState:update(dt)
    ...
    local diffY = math.abs(self.player.y - self.snail.y)
    ...
    if diffY > 5 * TILE_SIZE then
        self.snail:changeState('moving')
    elseif self.player.y < self.snail.y then
        self.snail.y = self.snail.y - SNAIL_MOVE_SPEED * dt
